...
@section('content')
<div class="space-y-6">
    <h2 class="text-2xl font-bold text-gray-800">Formulir Tambah Data Faskes</h2>

    <div class="max-w-2xl bg-white p-6 rounded-lg shadow">
        <form action="{{ route('kesehatan.store') }}" method="POST">
            @include('kesehatan._form', ['data' => []])
            <div class="pt-5 mt-5 border-t border-gray-200 flex justify-end space-x-3">
                <a href="{{ route('kesehatan.index') }}" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</a>
