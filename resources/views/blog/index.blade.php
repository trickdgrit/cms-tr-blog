...
<div class="space-y-8">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-sm text-gray-500">Kelola postingan dan berita terbaru.</p>
        </div>
    </div>
            ...
            <table class="min-w-full bg-white">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="py-3 px-4 border-b text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tanggal Publikasi</th>
                ...
                <tbody class="text-gray-700">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="py-3 px-4 border-b">{{ $item['judul'] ?? 'N/A' }}</td>
                        <td class="py-3 px-4 border-b">{{ $item['kategori'] ?? 'N/A' }}</td>
                        <td class="py-3 px-4 border-b">{{ isset($item['tanggal_publikasi']) ? \Carbon\Carbon::parse($item['tanggal_publikasi'])->format('d M Y') : 'N/A' }}</td>
                        <td class="py-3 px-4 border-b text-center">
                            @if(isset($item['id']))
                            <div class="flex item-center justify-center space-x-2">
                                <a href="{{ route('blog.edit', $item['id']) }}" class="text-yellow-500 hover:text-yellow-700">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" /><path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" /></svg>
                                </a>
                                <form action="{{ route('blog.destroy', $item['id']) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="text-center py-4">Tidak ada postingan.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
